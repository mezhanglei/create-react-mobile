 ...
 * 
 * 使用：1. 继承或实例化一个类WatchableStore: new WatchableStore()
 *      2. 监听目标事件，const id = WatchableStore.wath(目标事件)，返回一个id，此id用来卸载监听目标事件
 ...
 * 
 * 使用：1. 继承或实例化一个类WatchableStoreByType: new WatchableStoreByType()
 *      2. 监听目标事件，const type = WatchableStoreByType.wath(事件名, 目标事件)，返回一个type，此type用来卸载监听目标事件
