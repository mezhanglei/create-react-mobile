...
export function wxLink() {
  ...
  const redirect_uri = encodeURIComponent(location.origin + '#/' + "auth-wechat" + `?current=${encodeURIComponent(location.href)}`);
...
export function initWX() {
  ...
  if (!isInWeChat && process.env.NODE_ENV != "development") {
    const prefix = location.origin + '#/' + "not-wechat";
    // const prefix = location.origin + "not-wechat"; // history模式
...
export function handleRedirect() {
  ...
  if (code && splitArr && splitArr.length == 2) {
    const authUrl = location.origin + '#/' + splitArr[1];
